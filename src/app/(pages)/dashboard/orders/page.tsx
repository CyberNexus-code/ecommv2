...
export default async function clientOrders(){
    ...
    const now = Date.now();
    // Search and filter state (client-side only)
    const [search, setSearch] = useState("");
    const [statusFilter, setStatusFilter] = useState("");
    const filteredOrders = (orders ?? []).filter(order => {
      const matchesSearch = search === "" || order.customer_name?.toLowerCase().includes(search.toLowerCase()) || order.customer_email?.toLowerCase().includes(search.toLowerCase()) || String(order.order_number).includes(search);
